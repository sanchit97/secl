...
class BaseDATrainer:
    def __init__(self, loaders, args, backbone="resnet34"):
        ...
        self.model = SENNModel(backbone, output_dim=args.dataset["num_classes"]).cuda()
    ...
    def get_source_loss(self, step, *data):
        return self.model.base_source_loss(*data)
        # return self.model.base_loss(*data)

    def get_target_loss(self, step, *data):
        # self.model.f.requires_grad = False
        # self.model.concept_map.requires_grad = False
        return self.model.base_target_loss(*data)
        # return self.model.base_loss(*data)

    def get_ssl_loss(self,step, *data):
        return self.model.ssl_loss(*data)
    ...
    def logging(self, step, info, unit="min"):
        ...
        # torch.save(self.model.state_dict(), os.path.join(wandb.run.dir, "model-"+str(step)+".h5"))
    ...
    def training_step(self, step, *data):
        # sx, ssx, sy, tx, ttx, ty, _ = data
        sorig, sx1, sx2, sy, torig, tx1, tx2, ty, _ = data
        self.optimizer.zero_grad()
        # s_loss = self.get_source_loss(step, sx, sy)
        # t_loss = self.get_target_loss(step, tx, ty)

        s_loss = self.get_source_loss(step, sorig, sy)
        t_loss = self.get_target_loss(step, torig, ty)

        s_ssl_loss = self.get_ssl_loss(step, sx1, sx2)
        t_ssl_loss = self.get_ssl_loss(step, tx1, tx2)

        s_loss += 0.1*s_ssl_loss
        t_loss += 0.1*t_ssl_loss

        loss = (s_loss + t_loss) / 2
        loss.backward()
        self.optimizer.step()

        return s_loss.item(), t_loss.item(), 0, s_loss+t_loss
    ...
    def train(self):
        ...
        for step in range(1, self.config.num_iters + 1):
            ...
            # (sx, ssx, sy), (tx, ttx, ty), ux = next(self.iter_loaders)
            (sorig, sx1, sx2, sy), (torig, tx1, tx2, ty), ux = next(self.iter_loaders)
            # s_loss, t_loss, u_loss = self.training_step(step, sx, sy, tx, ty, ux)
            s_loss, t_loss, u_loss, ssl_loss = self.training_step(step, sorig, sx1, sx2, sy, torig, tx1, tx2, ty, ux)
